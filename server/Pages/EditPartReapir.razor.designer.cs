...
{
    public partial class EditPartReapirComponent : ComponentBase
    {
        ...
        {
            var simpleCarRepairDbGetPartReapirByIdResult = await SimpleCarRepairDb.GetPartReapirById(Id);
            partreapir = simpleCarRepairDbGetPartReapirByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(SimpleCarRepair.Models.SimpleCarRepairDb.PartReapir args)
        {
            try
            {
                var simpleCarRepairDbUpdatePartReapirResult = await SimpleCarRepairDb.UpdatePartReapir(Id, partreapir);
                DialogService.Close(partreapir);
            }
            catch (System.Exception simpleCarRepairDbUpdatePartReapirException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update PartReapir" });
            }
        }
