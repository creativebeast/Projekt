...
{
    public partial class EditRepairsComponent : ComponentBase
    {
        ...
        {
            var simpleCarRepairDbGetRepairByIdResult = await SimpleCarRepairDb.GetRepairById(Id);
            repair = simpleCarRepairDbGetRepairByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(SimpleCarRepair.Models.SimpleCarRepairDb.Repair args)
        {
            try
            {
                var simpleCarRepairDbUpdateRepairResult = await SimpleCarRepairDb.UpdateRepair(Id, repair);
                DialogService.Close(repair);
            }
            catch (System.Exception simpleCarRepairDbUpdateRepairException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Repair" });
            }
        }
